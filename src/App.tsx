    ...
    <div className="flex justify-center items-center h-screen">
      <ThemeProvider>
        <div className="flex flex-col justify-center items-center h-full w-full space-y-4">
          ...
          <div
            className="w-full max-w-2xl px-4 py-6 bg-zinc-100 shadow rounded-lg overflow-hidden flex flex-col"
          ...
          >
            <ScrollArea className="flex-1">
              {messages.map((message, index) => (
                <div
                  key={index}
                  className={`flex items-center ${message.isBot ? 'space-x-2 p-2 justify-start mr-2' : 'space-x-reverse space-x-2 p-2 justify-end mr-2'}`}
                >
                  {message.isBot ? (
                    <>
                      <Avatar>
                      ...
                      <div className="p-1 bg-white rounded-lg shadow text-black flex justify-between items-center">
                        {message.text}
                      </div>
                    </>
                  ) : (
